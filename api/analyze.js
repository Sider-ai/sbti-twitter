/**
 * SBTI Twitter Analysis API
 * Deploy as Cloudflare Worker or Node.js endpoint
 *
 * Input:  POST { tweets: string[], handle: string }
 * Output: { dimensions: {S1:"H",...}, analysis: string, evidence: string[] }
 *
 * TODO: Replace LLM_API_URL and LLM_API_KEY with your Sider LLM service config
 */
...
export default {
  async fetch(request, env) {
    ...
    try {
      const { tweets, handle, lang } = await request.json()
      ...
      const apiKey = env?.LLM_API_KEY || LLM_API_KEY
      const apiUrl = env?.LLM_API_URL || LLM_API_URL
      ...
      for (const d of dims) {
        if (!result.dimensions?.[d] || !validLevels.includes(result.dimensions[d])) {
          ...
          result.dimensions[d] = 'M' // Default to M if missing
        }
      }
